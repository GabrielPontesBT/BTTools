...
http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/api/analyze-workflow') {
    ...
    try {
      await new Promise((resolve, reject) => {
        let out = '';
        const child = spawn('node', ['scripts/js/generar_workflow.js', service, outputFile], { cwd });
    ...
    for (let i = 0; i < genItems.length; i++) {
      ...
      if (ejecutar && item.method === '__all__') {
        ...
        if (fs.existsSync(wfPath)) {
          args = ['scripts/js/generar_md.js', '--workflow', wfFile];
        } else {
          args = ['scripts/js/generar_md.js', item.service, '--ejecutar'];
        }
      } else {
        args = ['scripts/js/generar_md.js', item.service];
  ...
  if (req.method === 'POST' && req.url === '/api/validate-md') {
    try {
      ...
      const scriptPath = path.join(ROOT, 'tools', 'validar_md.js');
      if (!fs.existsSync(scriptPath)) {
        json(200, { ok: false, output: 'No se encontró validar_md.js en ' + path.join(ROOT, 'tools') });
        return;
      }
      const raw = await new Promise((resolve, reject) => {
